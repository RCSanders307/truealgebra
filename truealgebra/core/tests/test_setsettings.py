...
def test_set_default_bp(lbp, rbp, assignment, settings):
    setsettings.set_default_bp(lbp, rbp)
...
def test_set_custom_bp(name, lbp, rbp, assignment, settings):
    setsettings.set_custom_bp(name, lbp, rbp)
...
def test_set_infixprefix(name, rbp, assignment, extra, settings):
    if extra:
        exec(extra)
    setsettings.set_infixprefix(name, rbp)
...
def test_set_symbol_operators_errors(
    ...
    if extra:
        exec(extra)
    setsettings.set_symbol_operators(name, lbp, rbp)
    output = capsys.readouterr()
...
def test_set_bodied_functionss_errors(
    ...
    if extra:
        exec(extra)
    setsettings.set_bodied_functions(name, rbp)
    output = capsys.readouterr()
...
def test_set_container_subclass(name, cls, settings):
    setsettings.set_container_subclass(name, cls)
...
def test_set_complement(complementname, targetname, settings):
    setsettings.set_complement(complementname, targetname)
...
def test_set_category(category, name, catdict, settings):
    if name:
        setsettings.set_categories(category, name)
    else:
        setsettings.set_categories(category)
...
def test_set_parse(settings, afunc):
    setsettings.set_parse(afunc)

    assert settings.parse == afunc

def test_set_unparse(settings, afunc):
    setsettings.set_unparse(afunc)

    assert settings.unparse == afunc
